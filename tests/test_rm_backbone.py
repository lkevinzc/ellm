...
buffer_dict = {
    "v1": "/home/aiops/liuzc/ellm/output/online_SimPO_dump_0811T15:29/buffer.pkl",
    "v2": "/home/aiops/liuzc/ellm/output/bt_sample_dpo_online_passive_0907T06:48/all_buffer.pkl",
}


def main(
    model_name: str,
    buffer_version: str = "v1",  # llm-blender/PairRM-hf
    tag: str = "",
):
    buffer = pd.read_pickle(buffer_dict[buffer_version])
    data = []
    for b in buffer:
        data += list(b)

    _model_name = model_name.replace("/", "_")
    out_name = f"rm_features_{buffer_version}_{_model_name}{tag}.pt"
    print("output to ", out_name)

    backbone = DebertaV2PairRM.from_pretrained(model_name, device_map="cuda").eval()
    tokenizer = AutoTokenizer.from_pretrained(model_name)

    max_length = 2048
    source_max_length = 1224

    # backbone.cand_prefix_id = backbone.config.cand1_prefix_id
    # backbone.cand_prefix = "<candidate1>"
